...
** Build Docker image
#+name: docker
#+begin_src python :var directory="moonlander" :results output
print('      - name: Build qmk docker image')
print('        run: docker build -t qmk -f ' + directory + '/Dockerfile .')
#+end_src
...
print('        run: |')
print('          # Dynamically find where the keyboard folder actually lives in ZSA\'s QMK fork')
print('          if [ -d "qmk_firmware/keyboards/zsa/${{ github.event.inputs.layout_geometry }}" ]; then')
...
print('          ')
print('          # Handle Moonlander Rev A vs Rev B split for QMK compilation')
print('          make_target="${make_prefix}${{ github.event.inputs.layout_geometry }}"')
print('          if [ "${{ github.event.inputs.layout_geometry }}" = "moonlander" ]; then')
print('            make_target="${make_target}/reva"  # <--- Change this to /revb if your keyboard is newer than Spring 2024')
print('          fi')
print('          ')
...
print('            make ${make_target}:${{ github.event.inputs.layout_id }}')
print('          "')
print('          ')
print('          # Find and export built layout')
print('          normalized_layout_geometry="$(echo "${{ github.event.inputs.layout_geometry }}" | sed \'s|/|_|g\')"')
print('          ')
print('          # Debug: Show exactly where QMK dropped the binaries in the Action logs')
print('          find ./qmk_firmware -type f -name "*.bin" -o -type f -name "*.hex" -o -type f -name "*.uf2"')
print('          ')
print('          # Find the exact file without using fragile regex')
print('          found_file=$(find ./qmk_firmware -type f -name "*${normalized_layout_geometry}*.bin" -o -type f -name "*${normalized_layout_geometry}*.hex" -o -type f -name "*${normalized_layout_geometry}*.uf2" | head -n 1)')
print('          ')
print('          echo "built_layout_file=${found_file}" >> "$GITHUB_OUTPUT"')
...
<<docker(directory="moonlander")>>
<<build(directory="moonlander/firmware")>>
<<upload(artifact="moonlander")>>
#+end_src
** Planck EZ
